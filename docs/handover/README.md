...
| session-list-interaction.md | 会话列表交互 + 未读系统：hover 直显分屏/归档、右键完整菜单（含已读会话/硬删除）、`chat_sessions.unread` 列迁移 + `updateSessionUnread`（不写 updated_at 不重排）、客户端 stream-end 检测（prevStreamingRef diff、排除当前会话、bridge 盲区语义边界）、DELETE `?hard=true` 物理删除 vs 归档软删 |
