...
| session-list-interaction.md | 会话列表交互 + 未读系统：hover 直显分屏/归档、右键完整菜单（含已读会话/硬删除）、`chat_sessions.unread` 列迁移 + `updateSessionUnread`（不写 updated_at 不重排）、客户端 stream-end 检测（prevStreamingRef diff、排除当前会话、bridge 盲区语义边界）、DELETE `?hard=true` 物理删除 vs 归档软删。**迭代二**：归档即时化（去 confirm）、项目 = DB 有任意会话的目录（含归档）→ `getKnownProjects` + `GET /api/chat/projects` + 空文件夹注入，「Remove project」改硬删语义 |
| session-usage-cockpit.md | 会话级订阅用量 popover：按 Runtime+Provider 门控（codex/claude/none）、Claude 走内部 /api/oauth/usage（keychain/文件 token + 必须 curl 绕 Node TLS 403 + limits[] 解析 0–100/ISO）、诚实空态判别式、curl 依赖与端点未公开的降级 |
