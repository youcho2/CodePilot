...
| [active/context-accounting-runtime-contract.md](active/context-accounting-runtime-contract.md) | **Context Accounting Runtime Contract（v2 Codex review pass）**：Phase 6 Tier 2 真实 smoke 暴露 "Skills 数字是 hardcoded 不是真实 Skill 调用" 假数据问题；定义三 Runtime 统一 snapshot contract + Skills available vs loaded/invoked 强制区分 + producedBy 用 RuntimeId 不用 'native' 别名 + Codex 拆三场景 (codex_account / codepilot_proxy / native app-server) + Codex run_completed 必须落 result.usage.context_breakdown | 📋 讨论中（v2 接受 4 P1 + 1 P2 review findings）；Phase 0 止损 + Phase 1 Contract 等用户拍板 |
