...
| [active/sentry-telemetry-reliability.md](active/sentry-telemetry-reliability.md) | **Sentry 遥测可信度与错误分诊**：隔离官方 stable、development 与 Fork，统一 Browser/Next/Electron 初始化和匿名 opt-out，建立脱敏 allow-list、Outcome 分类、默认 stack / normalized fingerprint 双轨分组与三层 source-map 发布闭环；先阻断安装包携带 map，SDK 升级独立可回滚，真实产品缺陷在 72h 干净数据后分拆移交 | 🚧 v0.64.0 / v0.65.0 已正式发布，Phase 6 进行中；source map/native/package 已实证，4xx/DNS/NoOutput/info Issue P1 已 code complete 且本地 gates 通过；待新 stable 真实 smoke 与只读 token 复核线上数字 |
