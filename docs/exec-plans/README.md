...
| 文件 | 主题 | 状态 |
|------|------|------|
| [active/harness-home-user-owned-core.md](active/harness-home-user-owned-core.md) | **Harness Home Umbrella**：用户所有、与框架/模型解耦的领域聚合根；维护共享定义、v0.62 事实底座、Full Reference 约束、跨 program 依赖与 Shared Phase 0 门禁，不再混放工程、DB 和设计 R&D 的实施状态 | 🔄 第二轮 review 已回写；基线锁定，待 inventory、enforcement anchors、write model 与 SecretStore 决策 |
| [active/harness-home-core-adapters.md](active/harness-home-core-adapters.md) | **Program A — Harness Core / Repository / Adapter Kits**：file-backed canonical repository、单写者/事务写/外部编辑对账、SecretStore、L0/L1 HarnessAdapter conformance、L2/L3 RuntimeAdapter 与 CodePilot Full Reference | 📋 待 Shared Phase 0 门禁；未授权产品代码实施 |
| [active/harness-home-asset-library.md](active/harness-home-asset-library.md) | **Program B — Producer-backed Asset Library**：复用 Gallery/media pipeline，按真实 producer 注册 kind，完成 backfill、lineage、typed reference、HTML bundle materialization 与 trust/CSP；无 producer 的 component/document 不进首版 schema/UI | 📋 待 Program A shared contract；未授权产品代码实施 |
| [active/harness-home-design-method.md](active/harness-home-design-method.md) | **Program C — CodePilot Design Method**：从真实案例、反例和用户选择沉淀可版本化 Method；Taste Memory evidence-only、分 scope、可撤销；图片→视频→HTML 编排使用 producer-backed Asset lineage | 📋 真实案例采集待开始；未授权产品代码实施 |
...
| [active/model-capability-reasoning-refresh.md](active/model-capability-reasoning-refresh.md) | **模型目录与推理强度统一适配**：GLM-5.2、Kimi for Coding、GPT-5.6、Claude Sonnet 5 / Opus 5，以及 ClinePass / OpenCode Go 的显式 Kimi K3；按模型与网关真实 capability 驱动输入框右侧强度选择，不显示伪档位 | 🚧 Phase 0/1/2 恢复修复已落地；Opus 5 目录、1M context、三 Runtime共用 adaptive/effort、provenance 与精确档位合同完成；SDK 动态目录不再让真实 Opus 5 turn 回复后显示成 Default。packaged UI/output 与其他真实 provider effort smoke 待继续 |
