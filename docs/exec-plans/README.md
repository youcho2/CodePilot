...
| [active/preview-build-readiness.md](active/preview-build-readiness.md) | **分支预览包发布前收口**：不合 main，从当前 worktree 打 macOS + Windows Preview 包前的 blocker 修复、Windows readiness、macOS readiness、打包 smoke 与小范围试用闭环 | 📋 已 review，待执行（补 2 个 P1 打包前置：数据隔离 `~/.codepilot` + Windows 必须在 Windows 构建） |
