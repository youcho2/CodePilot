...
| [active/qwen-token-plan-and-grok-access.md](active/qwen-token-plan-and-grok-access.md) | **千问 Token Plan 与 Grok 4.5 接入**：把百炼 Coding Plan、千问 Token Plan 个人版/团队版建模为三个稳定套餐，修复同 URL identity 串线并增加交互式使用 gate；xAI 同时接 API Key + Grok 4.5 Responses 和兼容 SuperGrok browser/device OAuth；Phase 7 补齐两类凭据、两种 Runtime 的原生 X Search | 🟡 Phase 0–5 与 0.59.1 发布完成；Phase 7 code/tests 完成、xAI OAuth × CodePilot Runtime 真实 `x_search` 通过，其余组合与 packaged Sub-agent route 复验待跑 |
...
| [active/model-capability-reasoning-refresh.md](active/model-capability-reasoning-refresh.md) | **模型目录与推理强度统一适配**：GLM-5.2、Kimi for Coding、GPT-5.6、Claude Sonnet 5，以及 ClinePass / OpenCode Go 的显式 Kimi K3；按模型与网关真实 capability 驱动输入框右侧强度选择，不显示伪档位 | 🚧 Phase 0/1/2 恢复修复已落地，Kimi for Coding 为 Auto/Low/High/Max；ClinePass / OpenCode Go K3 direct smoke 通过，真实 provider effort smoke 待继续 |
