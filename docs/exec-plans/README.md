...
| 文件 | 主题 | 状态 |
|------|------|------|
| [active/harness-home-user-owned-core.md](active/harness-home-user-owned-core.md) | **Harness Home 用户所有核心层 × 轻量 Harness/Runtime 分级接入 × 通用创作资产闭环**：Harness Home 先定义为与 UI 无关的领域聚合根；CodePilot Channel 是 Full Reference Channel；拆分 HarnessAdapter 与 RuntimeAdapter，外部框架按 L0 Discover / L1 Portable Projection / L2 Execution Bridge / L3 Full Runtime 渐进接入；Gallery 演进为图片/视频/音频/网页/组件通用 Asset lineage；用户真实审美沉淀为可版本化 CodePilot Design Method + user/project overlay。本地 `main` 已从旧 divergent 0.58 基线规范化到正式 `v0.62.0` | 🔄 最新版基线已建立；待 Claude Code plan review |
