...
| 文件 | 主题 | 状态 |
|------|------|------|
| [active/harness-home-user-owned-core.md](active/harness-home-user-owned-core.md) | **Harness Home Umbrella**：用户所有、与框架/模型解耦的领域聚合根；维护共享定义、v0.62 事实底座、Full Reference 约束、跨 program 依赖与 Shared Phase 0 门禁，不再混放工程、DB 和设计 R&D 的实施状态 | 🔄 Shared Phase 0 与 Program A code/tests 完成；Program B/C 实施中 |
...
| [active/harness-home-asset-library.md](active/harness-home-asset-library.md) | **Program B — Producer-backed Asset Library**：复用 Gallery/media pipeline，按真实 producer 注册 kind，完成 backfill、lineage、typed reference、HTML bundle materialization 与 trust/CSP；无 producer 的 component/document 不进首版 schema/UI | 🔄 B0/B1 code/tests 完成；B2/B3 实施中 |
