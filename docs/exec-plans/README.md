...
| [active/ai-sdk-7-runtime-loop-adoption.md](active/ai-sdk-7-runtime-loop-adoption.md) | **AI SDK 7 Runtime 接入 + 自动化 Loop 实践**：把 AI SDK 7 接入拆成 Node22/Provider request-shape/ToolLoopAgent parity/安全小能力/迁移决策五段，并把自动化 Loop 限定在可机械验证、可暂停、可回滚的范围内 | 📋 计划已建，待 Claude Code 从 Phase 1 接手 |
