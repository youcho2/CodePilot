...
| [active/markdown-live-preview-and-file-tree.md](active/markdown-live-preview-and-file-tree.md) | **Markdown Live Preview 统一样式 × 文件树 Explorer 化 × 文件类型图标**：删 presentation 多主题并与聊天共享中立 Markdown contract；CM6 自研 Live Preview（最低渲染 parity 出货门禁）；文件树右键/重命名/删除（跨 owner file mutation coordinator + trash 打包 required checks）；material-icon-theme 静态子集 FileTypeIcon。用户要求撤回计划 commit 后的 Claude/loop 产物，已 reset 到 `089e4d45` 后由 Codex 重做；0.A state 层 11/11 通过，真实 DOM/IME 手工项待补 | 🚧 Phase 0 进行中（0.A partial） |
