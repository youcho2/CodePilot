...
| [active/model-capability-reasoning-refresh.md](active/model-capability-reasoning-refresh.md) | **模型目录与推理强度统一适配**：GLM-5.2、Kimi for Coding、GPT-5.6、Claude Sonnet 5；修复 Codex `model/list` schema drift，按模型真实 capability 驱动输入框右侧强度选择，不显示伪档位 | 📋 调研完成，待 Claude Code 实施 |
| [active/runtime-permission-modes.md](active/runtime-permission-modes.md) | **跨 Runtime 权限模式**：在“按规则询问 / 完全访问”之间加入真正的“替我审批”；Claude/Codex 接上游 auto reviewer，Native AI SDK 先做 fail-closed POC，不把 tool approval 或 bypass 冒充代审 | 📋 调研完成，待 Claude Code 实施 |
| [active/automatic-chat-titles.md](active/automatic-chat-titles.md) | **自动会话命名**：先统一两套首消息截断与 UI 刷新，再以同 session provider 后台生成语义标题；provenance/CAS 保证手动、系统、导入标题不被覆盖 | 📋 调研完成，待 Claude Code 实施 |
