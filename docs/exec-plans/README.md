...
| 文件 | 主题 | 状态 |
|------|------|------|
| [active/provider-secret-encryption-toggle.md](active/provider-secret-encryption-toggle.md) | **Provider 密钥加密开关**：fork 默认关闭 safeStorage 以消除 ad-hoc 签名每更新弹钥匙串；grandfather 老加密用户，关闭时反向迁移密文→明文（复用已加载 key，无额外弹窗），设置页开关 + API + DB 迁移 | 🟡 P0–P2 Code complete + Tests pass；packaged 验收待 y.4 |
