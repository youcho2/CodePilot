...
> 创建时间：2026-07-21
> 最后更新：2026-07-22
> 状态：🟡 Phase 0–5 代码与文档及 review gate 修复已完成；0.59.0 在另一台开启 HTTP system proxy 的 packaged macOS 上暴露浏览器成功、server token exchange 直连失败，0.59.1 已完成局部 env-proxy fetch、Claude release-gate 复审、自动化回归和 CI 同配置 macOS arm64 安装包验证，正式双平台 CI 门禁待标签触发。设备码、refresh/tool/effort、其他 Qwen 套餐和 0.59.1 packaged 双平台真实登录仍待验证，因此计划保持 active。
...
| Phase 6 | Tier 2 回归、真实凭据 smoke 与发布说明 | 🟡 0.59.1 发布门禁进行中，外部验收部分通过 | 0.59.1 proxy 定向 44/44、unit 4537/4537、Web smoke 19/19、build/electron:build 与 CI 同配置 macOS arm64 package 通过；双平台 CI 待完成，其余外部项保留待测 |
