# 合并前 Blocker：新建对话归属 + OpenRouter Claude Code 发送回归
...
| Phase 2 | 修新建对话归属语义 | 🔧 进行中 2026-06-03 | **本轮唯一目标**：补 assistant/file/folder/workspace 创建来源建模 + UI 入口；不只加按钮 |
| Phase 3 | UI 文案与恢复动作 | ⏸ 随 Phase 1 暂缓 | 真正不兼容（codepilot_runtime）的恢复提示已存在；本轮不做 |
| Phase 4 | 回归测试与 smoke | 🔧 仅覆盖 Phase 2 | 新建对话四种来源 + 不污染普通新对话；UI 入口 + CDP smoke |
...
> - 下一步（Step 2 UI）：侧栏助理/项目 `+` 写来源；文件树新增「在此新建对话」动作（区别于附加）；修污染（文件树选择不写全局上次目录）；聊天顶部显示归属；CDP smoke。
