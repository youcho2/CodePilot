...
- [x] **A5 [P2 debt] 权限超时静默拒绝 + cleanup 逻辑三处重复** — ✅ **完成 2026-06-19**（Step 1+2 均落地）
  ...
  - **A5 Step 2 follow-up（P3，Codex review 发现，2026-06-19 当日修）**：超时态新增后，侧栏"需要批准"徽章可能残留——徽章 = `pendingApprovalSessionIds.has(id)`（AppShell Set，源自 `pendingPermission && !permissionResolved`，超时时已正确转 false）**OR** 全局单值 `pendingApprovalSessionId`（入口1 超时 case 没清、入口2 只在 phase-changed/completed 清，超时走 snapshot-updated 漏掉）→ 全局来源残留 true，OR 进去徽章不灭。修：入口1 `permission_resolved` case 补 `setPendingApprovalSessionId('')`；入口2 `useStreamSubscription` 在 snapshot 显示 `pendingPermission && permissionResolved`（已决）时清全局（无条件清安全——AppShell 的 Set 独立重算，真正 pending 的 peer 靠 Set 仍亮）。顺带修掉同源的用户 resolve 残留。验证：typecheck + 全量单测；React 状态接线无 hook-test harness，靠 typecheck + Set 源独立保护 + 代码审查。
