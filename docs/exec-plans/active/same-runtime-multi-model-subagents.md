...
- [x] v0.60.0 OAuth route 回归：`xai-oauth` / `openai-oauth` 使用 picker、resolver、CodePilot/Codex managed Sub-agent 共享 catalog；未认证、disabled 与 Claude Code xAI 负例 fail closed，真实 packaged Grok child smoke 继续在 Ledger 单列。
...
| _待跑_ | packaged codepilot_runtime / codex_runtime | xAI OAuth | grok-4.5 | 用户已认证 SuperGrok | 更新后由 Grok 4.5 managed Sub-agent 执行一轮文本与一轮 `x_search` | 📋 | 需确认 route 在胶囊/详情中显示 `xAI Grok OAuth + Grok 4.5`，且真实 Provider 调用成功；不能用主会话 Grok 成功替代 |
