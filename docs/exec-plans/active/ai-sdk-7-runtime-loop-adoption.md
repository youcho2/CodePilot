...
| Phase 2 | Provider request-shape 能力矩阵 | 🚧 进行中 | 部分：fixture 矩阵 + targeted test 已完成（30+12 用例 / 29 sanitized fixture，见 [矩阵文档](../../research/ai-sdk-7-provider-request-shape-matrix.md)）；发现 3（.chat image 裸 base64）已本地收口（`withChatImageDataUrlFetch`，Codex 裁决 option a）；live smoke（本机 4 渠道，2026-07-03）已完成——reasoning/effort/tool choice 三条 wire 全部实测被接受，2 条网关 finding 转 tech-debt #47/#48（见 Smoke Ledger）；待：官方一方 Anthropic effort / OpenAI / 网关 image data URL 接受度（本机无官方渠道，open gate，是否以此结 Phase 2 交 Codex 裁决） |
...
| 2026-07-03 | ai-sdk-7 spike（`scripts/smoke-ai-sdk7-phase2-live.mjs`，镜像 app provider 构造，非 app 进程） | OpenCode Go (Anthropic)（anthropic Messages skin 网关） | glm-5 | DB `api_providers` readonly（x-api-key） | basic chat + thinking enabled(budget 1024) + `output_config.effort`(adaptive) | ✅（基础/thinking/effort 均 200 接受；effort 语义生效未证——reasoningText 0） | `chatcmpl-R8ye45HTW855FCSurjX6RYyJ` / `chatcmpl-RrV46vVDEoddPsIr1RE7kXni` / `chatcmpl-RQkYMwLNjgmT4T1b3fAxd3Eg`；**app 路径注意**：normaliseBaseUrl 保留深路径 → POST `/zen/go/messages` 404，需 base+`/v1`（tech-debt #47） |
...
| 2026-07-03 | ai-sdk-7 spike（app `.chat()` 网关路径） | ClinePass（api.cline.bot/api/v1） | anthropic/claude-haiku-4.5 等 4 个候选 | DB readonly（Bearer） | basic chat | ❌ 请求被接受但响应为非标准 `{"data":{…}}` 信封 → SDK "Invalid JSON response"（HTTP 200 已计费）；grok-code-fast-1 500×4（1+3 次指数退避重试）；`/v1` 变体 404 排除 base 配错 → tech-debt #48 | raw 证据 id `gen_01KWKEJ2MJT2FZ12MS1GCYT5TR`（envelope=wrapped-in-data） |
