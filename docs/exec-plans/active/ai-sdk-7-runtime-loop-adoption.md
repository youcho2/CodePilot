...
| Phase 2 | Provider request-shape 能力矩阵 | ✅ 已完成 | 三件套全部 Codex accepted：①fixture 矩阵（`3008c1a`，8/8 checks）②image 裸 base64 收口（Codex 仲裁选 a，`cc8b68e` wrapper+守卫+回归测试）③live smoke（`d4ec1723`，10/18 场景 200、reasoningTokens 语义实证、失败项如实归因，8/8 smoke checks，零 secret 泄漏）。**遗留 open gate（不阻塞收口，默认开放能力/发版前必补）**：官方一方 Anthropic/OpenAI key 的 smoke、网关 image data URL 接受度、ClinePass 500 待渠道恢复复测；Smoke Ledger 明细在 spike worktree 分支（待用户发起合并） |
