...
| [#628](https://github.com/op7418/CodePilot/issues/628) @file 误改上传副本 | 真实风险（fileResponseToAttachment 丢真实路径 → route 把 mention 也写 .codepilot-uploads 副本 → AI 改副本）；核心已修：FileAttachment 加 originPath、mention 保留真实路径、route 校验 cwd 内后引真实路径跳过 copy，AI Read/Edit 落真实文件 | P1-file-reference | 3 | 🟡 核心已修（待 Codex 复审 + 真机 smoke：@file 编辑落真实文件） |
