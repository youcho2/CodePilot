...
| [#629](https://github.com/op7418/CodePilot/issues/629) resume 400 空 assistant | 空 assistant 已防护；is_error result 坏 resume 不清 session id 的 gap 已修（POC-B 实证 4 proxy 文案 → 读 errors[] 喂 classifyError，补 `no conversation found` pattern，RESUME_FAILED/SESSION_STATE 才清、transient 保留） | P1-runtime-session | 2 | 🟢 已修（待 Codex 复审 + 真实 resume smoke） |
