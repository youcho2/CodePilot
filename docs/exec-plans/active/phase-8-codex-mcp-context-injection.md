...
| Widget | ✅ **机制接入（#31 首切片，2026-05-27）**：widget MCP 经泛化路由 `/api/codex/mcp/[server]` 注入（关键词门控，复用 `promptNeedsWidget`）；elicitation 审批接受（safe-read guidelines）；`show-widget` 渲染 runtime 无关（`parseAllShowWidgets`，已确认）；Codex Account 下从 perception_only 翻成可调用（带 caveat note）。route/注入/elicitation/翻转单测 + live 路由验证通过 | 待真实账号 smoke：widget-keyword 对话里模型自主调 guidelines + 输出可渲染 show-widget |
