...
| ST-15 | HTTP 4xx（含 429）、缺凭据、模型不支持 → `user_action_required`；5xx、DNS、timeout 只有 retry exhausted 才能上报 `transient_upstream`；NoOutput 必须先解包可信 cause/status/code，无 upstream 根因才归 `EMPTY_RESPONSE`。 |
...
- `src/lib/agent-loop.ts` + `src/lib/experimental/agent-loop-toolloop-poc.ts`：只在 retry-exhausted catch tail capture，`onError` 只保留结构化 root cause 并提前 marker。
...
- [ ] NoOutput wrapper 是否先解包 cause/status/code；新增字段是否在 allow-list、深度/节点/字符串预算内，且未读取 body/chunk/request/data？
...
- Native `onError` 不是 retry-exhausted boundary；直接在回调 capture 会把 SDK 后续 wrapper/catch 再报一次，并把未耗尽的 transient 提前变成 Issue。
...
- `telemetry-native-boundary-shape.test.ts`：Native/ToolLoop `onError` 延后到 terminal catch 与 shared marker 所有权。
...
- 2026-08-04：Phase 6 P1 统一 shared/native normalizer；429 随全部 4xx 固定为 user action，移除 `telemetry-health-v1.json`/info Issue 代码，Native 两条 loop 只在 terminal catch 上报；不新增 user/did、metrics 或行为遥测。
