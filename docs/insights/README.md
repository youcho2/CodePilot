...
| [session-list-interaction.md](./session-list-interaction.md) | [handover/session-list-interaction.md](../handover/session-list-interaction.md) | 会话列表交互 + 未读系统：高频操作前置(hover)/低频后置(右键)的分层理由、未读为何只标记"你不在场时完成的回复"而非所有活动、为何客户端检测并诚实标注 bridge 盲区(反假数据)、标记读/未读为何不重排序、硬删为何独立 + 强确认、已知局限(无计数/不跨设备) |
