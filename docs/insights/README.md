...
| [session-list-interaction.md](./session-list-interaction.md) | [handover/session-list-interaction.md](../handover/session-list-interaction.md) | 会话列表交互 + 未读系统：高频操作前置(hover)/低频后置(右键)的分层理由、未读为何只标记"你不在场时完成的回复"而非所有活动、为何客户端检测并诚实标注 bridge 盲区(反假数据)、标记读/未读为何不重排序、硬删为何独立 + 强确认、已知局限(无计数/不跨设备)。**迭代二**：可逆操作(归档)为何该即时而非弹确认、项目为何应锚定"DB 有无数据"而非"有无活跃会话"、Remove project 为何收敛为不可逆硬删、为何不用 localStorage 记项目 |
| [session-usage-cockpit.md](./session-usage-cockpit.md) | [handover/session-usage-cockpit.md](../handover/session-usage-cockpit.md) | 会话内查订阅用量：为什么按会话来源门控而非并列显示（反假数据）、为什么 Claude 敢做实时查询而非只用流式快照、curl vs Node TLS 403 与 minified 不能当契约的复盘、token 走 stdin 的安全细节 |
