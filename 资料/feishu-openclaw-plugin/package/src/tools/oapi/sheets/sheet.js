 ...
 *
 * 支持格式：
 *   https://bytedance.larkoffice.com/sheets/TOKEN
 *   https://xxx.feishu.cn/sheets/TOKEN?sheet=SHEET_ID
 *   https://xxx.larkoffice.com/wiki/TOKEN（知识库中的电子表格）
 */
...
async function resolveToken(p, client, log) {
    ...
    if (p.spreadsheet_token) {
        ...
        if (!parsed) {
            throw new Error(`无法从 URL 解析出 spreadsheet_token: ${p.url}`);
        }
        token = parsed.token;
...
const UrlOrToken = [
    Type.Optional(Type.String({
        description: "电子表格 URL，例如 https://xxx.larkoffice.com/sheets/TOKEN 或 https://xxx.larkoffice.com/wiki/TOKEN（与 spreadsheet_token 二选一）",
    })),
    Type.Optional(Type.String({
            ...
            try {
                ...
                switch (p.action) {
                    ...
                    case "read": {
                        ...
                        let range = p.range;
                        // 自动探测：没有 range 时，查询工作表列表获取 sheet_id
                        if (!range) {
                            const targetSheetId = p.sheet_id ?? urlSheetId;
                            if (targetSheetId) {
                                range = targetSheetId;
                            }
                            else {
                                // 查询第一个工作表
                                const sheetsRes = await client.invoke("feishu_sheet.read", (sdk, opts) => sdk.sheets.spreadsheetSheet.query({ path: { spreadsheet_token: token } }, opts), { as: "user" });
                                assertLarkOk(sheetsRes);
                                const firstSheet = (sheetsRes.data?.sheets ?? [])[0];
                                if (!firstSheet?.sheet_id) {
                                    return json({ error: "电子表格中没有工作表" });
                                }
                                range = firstSheet.sheet_id;
                                log.info(`read: auto-detected sheet "${firstSheet.title}" (${range})`);
                            }
                        }
                        ...
                        return json({
                            ...
                            values,
                            ...(truncated
                                ? { truncated: true, total_rows, hint: `数据超过 ${MAX_READ_ROWS} 行，已截断。请缩小 range 重新读取。` }
                                : {}),
                        });
                    }
                    // -----------------------------------------------------------------
                    ...
                    case "write": {
                        ...
                        if (p.values && p.values.length > MAX_WRITE_ROWS) {
                            return json({ error: `写入行数 ${p.values.length} 超过限制 ${MAX_WRITE_ROWS}` });
                        }
                        ...
                        let range = p.range;
                        if (!range) {
                            const sheetId = p.sheet_id ?? urlSheetId;
                            if (sheetId) {
                                range = sheetId;
                            }
                            else {
                                const sheetsRes = await client.invoke("feishu_sheet.write", (sdk, opts) => sdk.sheets.spreadsheetSheet.query({ path: { spreadsheet_token: token } }, opts), { as: "user" });
                                assertLarkOk(sheetsRes);
                                const firstSheet = (sheetsRes.data?.sheets ?? [])[0];
                                if (!firstSheet?.sheet_id) {
                                    return json({ error: "电子表格中没有工作表" });
                    ...
                    case "append": {
                        const { token, urlSheetId } = await resolveToken(p, client, log);
                        if (p.values && p.values.length > MAX_WRITE_ROWS) {
                            return json({ error: `追加行数 ${p.values.length} 超过限制 ${MAX_WRITE_ROWS}` });
                        }
                        let range = p.range;
                        if (!range) {
                            const sheetId = p.sheet_id ?? urlSheetId;
                            if (sheetId) {
                                ...
                                const sheetsRes = await client.invoke("feishu_sheet.append", (sdk, opts) => sdk.sheets.spreadsheetSheet.query({ path: { spreadsheet_token: token } }, opts), { as: "user" });
                                assertLarkOk(sheetsRes);
                                const firstSheet = (sheetsRes.data?.sheets ?? [])[0];
                                if (!firstSheet?.sheet_id) {
                                    return json({ error: "电子表格中没有工作表" });
                    ...
                    case "create": {
                        ...
                        if (!token) {
                            return json({ error: "创建表格失败：未获取到 token" });
                        }
                                ...
                                if (firstSheet?.sheet_id) {
                                    ...
                                    if (writeRes.code && writeRes.code !== 0) {
                                        ...
                                        return json({
                                            spreadsheet_token: token,
                                            url,
                                            warning: `表格已创建，但写入初始数据失败：${writeRes.msg}`,
                                        });
                                    }
                                }
                            }
                    ...
                    case "export": {
                        ...
                        if (p.file_extension === "csv" && !p.sheet_id) {
                            return json({
                                error: "导出 CSV 必须指定 sheet_id（CSV 一次只能导出一个工作表）。请先用 info 获取工作表列表。",
                            });
                        }
                        ...
                        if (!ticket) {
                            return json({ error: "导出任务创建失败：未获取到 ticket" });
                        }
                        ...
                        for (let i = 0; i < EXPORT_POLL_MAX_RETRIES; i++) {
                            ...
                            if (jobStatus !== undefined && jobStatus >= 3) {
                                return json({ error: result?.job_error_msg || `导出失败 (status=${jobStatus})` });
                            }
                            log.info(`export: polling ${i + 1}/${EXPORT_POLL_MAX_RETRIES}, status=${jobStatus}`);
                        }
                        if (!fileToken) {
                            return json({ error: "导出超时：任务未在 30 秒内完成" });
                        }
