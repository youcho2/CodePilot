...
describe('isolated Sentry telemetry smoke', () => {
  ...
  it('compiles the fixture only for an explicit manual macOS smoke', () => {
    ...
    // Fork ships macOS arm64 only — the Windows/Linux jobs (which pinned the
    // smoke flag off) were removed. The packaged telemetry smoke steps stay
    // gated on the manual workflow_dispatch input, so a tag push never runs them.
    assert.match(
      workflow,
      /Run packaged Sentry synthetic events\n\s*if:\s*\$\{\{ github\.event_name == 'workflow_dispatch' && inputs\.telemetry_smoke \}\}/,
    );
