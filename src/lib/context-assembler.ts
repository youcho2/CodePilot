...
\`\`\`show-widget
{"title":"hatch_buddy","widget_code":"<div style='text-align:center;padding:32px 16px;font-family:system-ui'><div id='egg-container'><div style='font-size:64px;animation:wobble 1s ease-in-out infinite alternate'>🥚</div></div><style>@keyframes wobble{0%{transform:rotate(-5deg)}100%{transform:rotate(5deg)}}</style><p style='margin:16px 0 8px;font-size:15px;font-weight:600'>你的伙伴正在等待孵化！</p><p style='font-size:13px;color:#888;margin:0 0 20px'>每个助理都有一个专属伙伴<br/>它会随着你们的互动一起成长和进化</p><button id='hatch-btn' onclick=\\"window.parent.postMessage({type:'widget:hatch-buddy'},'*');this.textContent='孵化中…';this.disabled=true\\" style='background:#6C5CE7;color:white;border:none;padding:10px 32px;border-radius:20px;font-size:14px;cursor:pointer;font-weight:500'>🐣 孵化伙伴</button><script>window.addEventListener('message',function(e){if(e.data&&e.data.type==='hatch-buddy:result'&&e.data.buddy){var b=e.data.buddy;var r=b.rarity==='common'?'★ 普通':b.rarity==='uncommon'?'★★ 稀有':b.rarity==='rare'?'★★★ 精良':b.rarity==='epic'?'★★★★ 史诗':'★★★★★ 传说';document.getElementById('egg-container').innerHTML='<div style=font-size:64px>'+b.emoji+'</div>';document.getElementById('hatch-btn').style.display='none';var p=document.createElement('div');p.innerHTML='<p style=font-size:15px;font-weight:600;margin:16px>🎉 孵化成功！</p><p style=font-size:13px;color:#888>'+b.emoji+' '+r+'</p>';document.getElementById('egg-container').parentNode.appendChild(p)}})</script></div>"}
\`\`\`
